...
class KeyValueUserScopePermissionsResolverTestCase(KeyValuePermissionsResolverTestCase):
    ...
    def test_user_permissions_for_system_scope_kvps(self):
        resolver = KeyValuePermissionsResolver()

        # Setup user, grant, role, and assignment records
        user_db = UserDB(name="user_key1")
        user_db = User.add_or_update(user_db)
        self.users[user_db.name] = user_db

        kvp_1_db = KeyValuePairDB(
            uid="%s:%s:%s" % (ResourceType.KEY_VALUE_PAIR, FULL_SYSTEM_SCOPE, "key1"),
            scope=FULL_SYSTEM_SCOPE,
            name="key1",
            value="myval1",
        )
        ...
        role_1_db = RoleDB(
            name="user_has_system_key1_create_grant",
            permission_grants=[str(grant_1_db.id)],
        )
        role_1_db = Role.add_or_update(role_1_db)
        self.roles[role_1_db.name] = role_1_db

        role_2_db = RoleDB(
            name="user_has_system_key1_list_grant",
            permission_grants=[str(grant_2_db.id)],
        )
        role_2_db = Role.add_or_update(role_2_db)
        self.roles[role_2_db.name] = role_2_db
        ...
        # User should have access to system scope to create user
        self.assertUserHasResourceDbPermission(
            resolver=resolver,
            user_db=user_db,
            resource_db=kvp_1_db,
